    ...
    SELECT
      (SELECT COALESCE(SUM(grand_total),0)
       ...
       WHERE business_unique_code=$1
         AND invoice_date=CURRENT_DATE) AS today_sales,

      (SELECT COALESCE(SUM(balance_after),0)
       FROM ekbill.customer_ledger
       WHERE business_unique_code=$1) AS pending_payments,

      (SELECT COUNT(*)
       FROM ekbill.products p
       JOIN (
         SELECT product_unique_code,business_unique_code,
                SUM(CASE WHEN transaction_type IN ('OPENING','IN') THEN quantity
                         WHEN transaction_type='OUT' THEN -quantity END) AS stock
         FROM ekbill.product_stock_history
         GROUP BY product_unique_code,business_unique_code
       ) s ON s.product_unique_code=p.product_unique_code
       AND s.business_unique_code=p.business_unique_code
       WHERE p.business_unique_code=$1
         AND p.low_stock_alert IS NOT NULL
         AND s.stock <= p.low_stock_alert) AS low_stock_items,

      (SELECT COUNT(DISTINCT customer_unique_code)
       FROM ekbill.customer_ledger
       WHERE business_unique_code=$1
         AND balance_after > 0) AS payment_reminders
