...
import {
  ...
  getMinimalCustomerList,
  deleteCustomer,
  updateCollectionReminderDate,
...
export const createCustomerService = async (data) => {
  ...
  try {
    // Upsert customer with mandatory fields only
    ...
    // Insert optional customer details if any field is provided
...
export const updateCustomerService = async (data) => {
  ...
  try {
    // Update core customer fields (mandatory)
    ...
    // Update optional customer details
    ...
    if (address) {
      await replaceCustomerAddress(customer_unique_code, user_unique_code, address);
    }
...
export const getMinimalCustomerListService = async (business_unique_code) => {
  try {
    return await getMinimalCustomerList(business_unique_code);
  } catch (error) {
    console.error("getMinimalCustomerListService error:", error);
    throw new Error(error.message || "Failed to fetch customer list");
  }
};
...
export const updateCollectionReminderDateService = async (
  ...
  try {
    ...
    console.error("updateCollectionReminderDateService error:", error);
...
export const addCustomerLedgerEntryService = async (data) => {
  ...
  try {
    ...
    console.error("addCustomerLedgerEntryService error:", error);
    throw new Error(error.message || "Failed to add ledger entry");
  } finally {
    client.release();
  }
};
...
export const getCustomerDetailsService = async (
  ...
  try {
    ...
    if (!business_unique_code) {
      throw new Error("Business code is missing");
    }
    ...
    console.error("getCustomerDetailsService error:", error);
